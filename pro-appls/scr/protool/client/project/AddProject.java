...
public class AddProject extends JFrame implements ActionListener{
    ...
    public AddProject(String title, int proId){
   	super(title); 
...
public class AddAddress extends JFrame implements ActionListener, ItemListener{//implements ActionListener{
		...
		GridBagConstraints billPane = new GridBagConstraints();
		 billPane.gridx = 0;
		 billPane.gridy = 0;
		 billPane.insets = new Insets(-20, 0, 0, 0);
	...
	if (e.getSource().equals(address)){
	      new AddAddress("New Contact Details");	
	}
	...
	if (e.getSource().equals(save)){
		...
		ArrayList<Object> projectQuote = ul.getArrayList(bean.connect().addProject(ul.writeArrayList(prodetails), ul.writeListOfArrayListStr(outerlist))); 
