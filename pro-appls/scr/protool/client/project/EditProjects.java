...
* @version 1.0
*/
public class EditProjects extends JFrame implements ActionListener{
    ...
    public EditProjects(String title, int proId, int quoteId){
   	super(title); 
    ...
    public EditQuotes(String title, int quoteId) {
	super(title);
	...
	public JPanel siteAddress(){
		...
		contact1.setText(siteDetails.get(6));
		pane.add(contact1);
		...
		mobile1.setText(siteDetails.get(7));
		pane.add(mobile1);
	...
	if (e.getSource().equals(quotebutton)){
	     EditQuotes ds = new EditQuotes("Edit Quote Details", quoteId);	
	}
	...
	if (e.getSource().equals(save)){
		...
		else{prodetails.add(addressinfo(""));}
		...
		prodetails.add("30 days");//Payment Term moved to PaymentIn view - sh.dayString(duration)8
		...
		if(contact1 != null)
			prodetails.add(contact1.getText());
		else{prodetails.add("");}

		if(mobile1 != null)
			prodetails.add(mobile1.getText());
		else{prodetails.add("");}	
		
		String[] genericInfo = {String.valueOf(quoteId), ul.usDateString(shInner.dateString(validDate))};
		
		bean.connect().updateProjectQuote(ul.writeArrayList(prodetails), ul.writeStringArray(genericInfo), ul.writeListOfArrayListStr(outerlist)); 
	  } catch (Exception ex) { 
		JOptionPane.showMessageDialog(this, "Quote PDF not generated!" +ex.toString(),   
                        "Error!", JOptionPane.INFORMATION_MESSAGE);
