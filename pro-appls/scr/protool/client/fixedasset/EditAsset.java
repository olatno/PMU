...
* @version 1.0
*/
public class EditAsset extends JFrame implements ActionListener, MouseListener{//implements ActionListener{
	...
	private boolean assetChange = false;
    ...
    public EditAsset(String title, int assetId){//, int proId
   	super(title); 

	this.assetId = assetId;
	//addpanel = new JPanel();
	bean = new BeanStud();
	details = pu.getArrayList(bean.connect().getEditAssetDetails(assetId));
	buttonpanel = new JPanel();
	generalPanel = new JPanel();
	sh = new SpinnerHelper();
	periodsh = new SpinnerHelper();
	assetAccountCode = String.valueOf(details.get(15));
	assetSupplierId = (Integer)details.get(16);
	insuranceSupplierId = (Integer)details.get(17);
	//Panels
	...
	public class AssetList extends JFrame implements ActionListener{//implements ActionListener{
    ...
    public AssetList(String title){//, int proId
   	super(title); 
	...
	public int getTableId(){
		int  maintablerow = generalView.getSelectedRow();
		return ((Integer)generalDataModel.getValueAt(maintablerow, 0)).intValue();
	}
	
	
	public String getStringValue(){
		int  maintablerow = generalView.getSelectedRow();
		return (String)generalDataModel.getValueAt(maintablerow, 1);
	}
   ...
   public void actionPerformed( ActionEvent e) {  
		...
		if(e.getSource().equals(go)){
			assetChange = true;
			assetAccountCode = String.valueOf(getTableId());
			//newAccount = getStringValue();
			name.setText(getStringValue());
			dispose();
		...
		if (e.getSource().equals(refresh)){
			dispose();	
			new AssetList("new General Expenses");
		}
	...
	public JPanel rightPanel(){
		...
		if(!rightList.isEmpty()){
			...
			supplierName.setText(String.valueOf(details.get(7)));
		}
		...
		if(!rightList.isEmpty()){
			...
			number.setText(String.valueOf(details.get(8)));
		}
		...
		if(!rightList.isEmpty()){
			...
			insurance.setText(String.valueOf(details.get(9)));
		}
		...
		if(!details.isEmpty()){
			addRight.add(sh.getGenericDateEdit(date, ul.ukFormat((java.sql.Date)details.get(10))), date1TextField);
		}
		else{
		...
		if(!details.isEmpty()){
			addRight.add(periodsh.getGenericSpinnerEdit(period, periodsh.getYearModel(), 
						String.valueOf(details.get(11))), periodTextField);
		}
		...
		if(!rightList.isEmpty()){
			...
			depreciation.select(String.valueOf(details.get(13)));
		}
	   ...
	   GridBagConstraints rateValLabel = new GridBagConstraints();
		...
		if(!rightList.isEmpty()){
			...
			rateValue.setText(String.valueOf(details.get(14)));
		}
	...
	public JPanel leftPanel(){
		...
		if(!leftList.isEmpty()){
			name.setText(leftList.get(8));
		}
		else if(!details.isEmpty()){
			name.setText(String.valueOf(details.get(0)));
		}
		...
		if(!leftList.isEmpty()){
			...
			description.setText(String.valueOf(details.get(1)));
		}
		...
		if(!leftList.isEmpty()){
			...
			group.select(String.valueOf(details.get(4)));
		}
		...
		if(!leftList.isEmpty()){
			...
			amount.setText(((BigDecimal)details.get(5)).toString());
		}
		...
		if(!leftList.isEmpty()){
			...
			vat.select(String.valueOf(details.get(6)));
		}
		addpanel.add(vat, vatTextField);
		
		newAsset.addActionListener(this);
		return addpanel;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource().equals(firstLabel) && leftSide){
			...
			rightList.add(String.valueOf(insuranceSupplierId));//insurance can be null value , we can use 29999 supplier id
    ...
    public void actionPerformed( ActionEvent e) {  
	 ...
	 if (e.getSource().equals(newAsset)){
		new AssetList("new General Expenses");
	 }
	 ...
	 if(!leftSide){
		...
		details.add(0, assetAccountCode); 
		...
		details.add(6, vat.getSelectedItem());
	 	...
	 	leftList.remove(leftList.get(8));
		...
		details.add(String.valueOf(insuranceSupplierId));//insurance can be null value, we can use 29999 supplier id
		...
		details.add(rateValue.getText());
	 	...
	 	bean.connect().editAsset(ul.writeArrayListStr(details), ul.writeInteger(Integer.valueOf(assetId))); 
	    dispose();	
	 }
