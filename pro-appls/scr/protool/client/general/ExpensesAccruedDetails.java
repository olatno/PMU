...
public class ExpensesAccruedDetails extends  JFrame implements ActionListener, ListSelectionListener{//ItemListener implements Serializable, ActionListener
    ...
    public ExpensesAccruedDetails(String title, byte[] obj, GeneralExpensesView view) {
	...
	ledgerTablePanel.add(new JLabel(String.format("%s %s", "Invoice Status", view.getStringValue(4,view.getModelAccrued(), view.getTableAccrued()))),BorderLayout.NORTH);
	...
	pu.paymentOutAccruedDetails(ledgerDataModel, obj, view);
	/*DefaultTableCellRenderer adjusmentCell = (DefaultTableCellRenderer)ledgerView.getCellRenderer(1, 4);
	DefaultTableCellRenderer render = new DefaultTableCellRenderer();
	BigDecimal adjamount = new BigDecimal(ul.noPoundString((String)ledgerDataModel.getValueAt(1, 4)));
	if(adjamount.compareTo(new BigDecimal("0")) < 0)
		adjusmentCell.setForeground(Color.decode("#FF0000")); */
    }
	...
	if (e.getSource().equals(exit)){
	 ...
	 if (e.getSource().equals(refresh)){
		 byte[] accruedResult =  bean.connect().getAccruedDetails(((Integer)ledgerDataModel.getValueAt(0, 5)).intValue());
		 pu.paymentOutAccruedDetails(ledgerDataModel, accruedResult);
	  }
    ...
    public void valueChanged(ListSelectionEvent e) {
		int  maintablerow = ledgerView.getSelectedRow();
		 byte[] accruedResult =  bean.connect().getAccruedDetails(((Integer)ledgerDataModel.getValueAt(0, 5)).intValue());
		if(maintablerow == 1 && pu.listOfObject(accruedResult).size() > 1){
		
			type.setText(String.valueOf(pu.listOfObject(accruedResult).get(3)));
			note.setText(String.valueOf(pu.listOfObject(accruedResult).get(4)));
		}
		else{
			type.setText("");
			note.setText("");
