...
* @version 1.0
*/
public class AddExpensesPayable extends JFrame implements ActionListener{//implements ActionListener{
    ...
    public AddExpensesPayable(String title, GeneralExpenses expenses){//, int proId
   	super(title); 
   ...
   public void actionPerformed( ActionEvent e) {  
		...
		if (e.getSource().equals(ok)){
			ArrayList<Object> payable = new ArrayList<Object>();
			payable.add(supplierIds);
			payable.add(Integer.valueOf(expenses.getTableId(0, expenses.getModelGeneral(),
				expenses.getTableGeneral())));
			payable.add(number.getText());
			payable.add(ul.usDateString(sh.dateString(date)));
			payable.add(amouth.getText());
			payable.add(vat.getSelectedItem());
			bean.connect().addExpensesPayable(ul.writeArrayList(payable)); 
			dispose();	
		}
	}
