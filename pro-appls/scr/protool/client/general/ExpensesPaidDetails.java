...
public class ExpensesPaidDetails extends  JFrame implements ActionListener, ListSelectionListener{//ItemListener implements Serializable, ActionListener
	...
	private int invoice;
	private byte[] obj;
	//private byte[] accruedResult;
	//private DefaultTableModel model;
	//private	JTable table;
	private PaymentInUtilities pu = new PaymentInUtilities();
	private BeanStud bean;
    private final String[]columnNames = {"DATE","PAYMENT(£)","VAT(£)","DATE","Amount(£)", " "};//"DATE","RECEIPT(£)","QUOTE(£)"
   

    public ExpensesPaidDetails(String title, byte[] obj, GeneralExpensesView view) {
	...
	ledgerTablePanel.add(new JLabel(String.format("%s %s", "Invoice Number", view.getStringValue(2, view.getModelPaid(), view.getTablePaid()))),BorderLayout.NORTH);
	...
	pu.paymentOutPaidDetails(ledgerDataModel, obj, view);
    }
	...
	if (e.getSource().equals(exit)){
	 ...
	 if (e.getSource().equals(refresh)){
		 ...
		 pu.paymentOutPaidDetails(ledgerDataModel, obj);
	  }
    ...
    public void valueChanged(ListSelectionEvent e) {
		...
		if(maintablerow > 0 &&  maintablerow <= 1){
			...
			type.setText(String.valueOf(pu.listOfObject(obj).get(3)));
			note.setText(String.valueOf(pu.listOfObject(obj).get(4)));
		}
		else{
			type.setText("");
			note.setText("");
