...
* @version 1.0
*/
public class PayableExpenseslPayment extends JFrame implements ActionListener{//implements ActionListener{
    ...
    public PayableExpenseslPayment(String title, String supplier, String invoice, int invoiceIds){
   	super(title); 
	...
	invoiceAmount.setText(String.valueOf(ul.getBigDecimals(bean.connect().getPayableBalance(invoiceIds))));
   ...
   public void actionPerformed( ActionEvent e) {  
	...
	if (e.getSource().equals(exit)){
	 ...
	 if (e.getSource().equals(ok)){
		...
		if(bank.getSelectedItem().equals("Credit Note")){
			prodetails.add("99999");
		}
		else{
			prodetails.add(String.valueOf(accname.get(bank.getSelectedItem())));
		}
		prodetails.add(bank.getSelectedItem());
		prodetails.add(note.getText());
		prodetails.add(ul.usDateString(sh.dateString(date)));
	    bean.connect().addPayableExpensesPayment(ul.writeArrayListStr(prodetails)); 
		dispose();
