...
* @version 1.0
*/
public class AddExpensesPaid extends JFrame implements ActionListener{//implements ActionListener{
    ...
    public AddExpensesPaid(String title, GeneralExpenses expenses){//, int proId
   	super(title); 
	...
	supplierName.setText(defaultstr.get(0));
	...
	addpanel.add(new JLabel("Receipt Number:"), numberLabel);
   ...
   public void actionPerformed( ActionEvent e) {  
	...
	if (e.getSource().equals(ok)){
	    ArrayList<Object> paid = new ArrayList<Object>();
		if(supplierName.getText().equals(defaultstr.get(0))){
			 paid.add(Integer.valueOf(defaultstr.get(1)));
		}
		else{
			paid.add(supplierIds);
		}
		paid.add(Integer.valueOf(expenses.getTableId(0, expenses.getModelGeneral(),
				expenses.getTableGeneral())));
		paid.add(number.getText());
		paid.add(ul.usDateString(sh.dateString(date)));
		paid.add(amount.getText());
		paid.add(accname.get(group.getSelectedItem()));
		paid.add(vat.getSelectedItem());
	    bean.connect().addExpensesPaid(ul.writeArrayList(paid)); 
		dispose();
	}
  }
