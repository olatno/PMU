...
* @version 1.0
*/
public class AddExpensesPrepaid extends JFrame implements ActionListener{//implements ActionListener{
    ...
    public AddExpensesPrepaid(String title, GeneralExpenses expenses){//, int proId
   	super(title); 
	...
	addpanel.add(new JLabel("Period:"), periodLabel);
	...
	periodTextField.ipadx = 50;
	periodTextField.anchor =  GridBagConstraints.WEST;
	addpanel.add(periodsh.getMonthSpinner(period), periodTextField);
...
//	group.add("Credit Card");
   ...
   public void actionPerformed( ActionEvent e) {  
	 ...
	 if (e.getSource().equals(ok)){
		java.util.List<Object> prepaid = new ArrayList<Object>();
		prepaid.add(supplierIds);
		prepaid.add(Integer.valueOf(expenses.getTableId(0, expenses.getModelGeneral(),
				expenses.getTableGeneral())));
		prepaid.add(number.getText());
		prepaid.add(ul.usDateString(sh.dateString(date)));
		prepaid.add(periodsh.monthString(period));
		prepaid.add(amount.getText());
		prepaid.add(accname.get(group.getSelectedItem()));
		prepaid.add(vat.getSelectedItem());
	    bean.connect().addExpensesPrepaid(ul.writeArrayList(prepaid)); 
	    dispose();	
	 }
