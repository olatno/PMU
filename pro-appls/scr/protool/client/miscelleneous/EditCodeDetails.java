...
* @version 1.0
*/
public class EditCodeDetails extends JFrame implements ActionListener, ItemListener{//implements ActionListener{
    ...
    public EditCodeDetails(int accountsCode, String title){//, int proId
   	super(title); 
	bean = new BeanStud();
	
	ArrayList<Object> ls = ul.getProjectForEditing(bean.connect().getAccountsDetails(accountsCode));
	...
	costId.setText(String.valueOf(ls.get(0)));
	...
	String editgroup = (String)ls.get(1);
	group.add(editgroup);
	for(int i = 0; i < groupbyname.length; i++){
	    if(!editgroup.equals((String)groupbyname[i])){
	...
	code.setText(String.valueOf(accountsCode));
	...
	name.setText(String.valueOf(ls.get(2)));
	...
	addpanel.add(sh.getGenericDateEdit(date, ul.ukFormat((java.sql.Date)ls.get(3))), date1TextField);
	...
	String state = (String.valueOf(ls.get(4)));

	if(state.equals("YES")){
		active = new JCheckBox("",true);
	}
	else{
		active = new JCheckBox("",false);
	}
	incField.gridx = 1;
	incField.gridy = 5;
	incField.ipady = 2;
	...
	boolean isManual = ((Boolean)ls.get(5)).booleanValue();

	if(!isManual){
		manual = new JCheckBox("",false);
	}
	else{
		manual = new JCheckBox("",true);
	}
	manualField.gridx = 1;
	manualField.gridy = 6;
	manualField.ipady = 2;
	...
	public void itemStateChanged(ItemEvent e) {
		...
		int groupid = initialAccountsCode(initialstr).intValue();
	...
	if (e.getSource().equals(update)){
	    ...
	    bean.connect().updateAccountsCode(ul.writeArrayListStr(accdetails)); 
		dispose();		
	   }
	 }
