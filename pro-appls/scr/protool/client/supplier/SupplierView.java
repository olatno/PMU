...
* @version 1.0
*/
public class SupplierView extends JPanel implements  ActionListener{// implements Serializable, ActionListener{
    ...
    private final String[]columnNames = {"COMPANY NAME","ACCOUNT NUMBER","ACCOUNT BALANCE","CREDIT LIMIT", "PAYMENT TERM","DATE CREATED", ""};
   ...
   //Panels
	...
	pu.loadData(projectDataModel,  bean.connect().getSupplierData());
	...
	public void actionPerformed( ActionEvent e) {  
			...
			if (e.getSource().equals(searchproject)){ 
				...
				if(balance_from.getText().equals("")&&balance_to.getText().equals("")&&suppliername.getText().equals("")){
					pu.loadSingleData(projectDataModel,  bean.connect().getSupplierDataId(supplier.getText()));
	 
				}
				else if(balance_from.getText().equals("")&&balance_to.getText().equals("")){
					supplier.setText("");
					pu.loadSingleData(projectDataModel,  bean.connect().getSupplierDataName(suppliername.getText()));
				}
	 
				else{
					supplier.setText("");
					suppliername.setText("");
					pu.loadData(projectDataModel,  bean.connect().getSupplierDataBalance(new BigDecimal(balance_from.getText()), new BigDecimal(balance_to.getText())));
				}
			...
			if(e.getSource().equals(detail)){
				...
				SupplierDetails ds = new SupplierDetails("Supplier Details", ul.getTableInt(projectView,projectDataModel,6));
			}
		
			if(e.getSource().equals(invoice)){
				AddSupplierInvoice ds = new AddSupplierInvoice("SUPLIER INVOICE", ul.getTableInt(projectView,projectDataModel,6));
			}
	   
			if(e.getSource().equals(batch)){
				BatchPayment bp = new BatchPayment("Invoice Batch Payment", ul.getTableInt(projectView,projectDataModel,6));
			}
