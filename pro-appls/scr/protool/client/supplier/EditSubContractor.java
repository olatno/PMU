...
* @version 1.0
*/
public class EditSubContractor extends JFrame implements ActionListener{
	...
	private int sub_contractid;
    
    public EditSubContractor(String title, int sub_contractid){
   	super(title); 
	...
	this.sub_contractid = sub_contractid;
	//address = new String();
	sh = new SpinnerHelper();
	pu = new PaymentInUtilities();
	ArrayList<Object> details = pu.getArrayList(bean.connect().getSubContractorSingleDetails(sub_contractid));
	...
	companyName.setText(String.valueOf(details.get(3)));
	...
	businessType.select(String.valueOf(details.get(4)));
	type.gridx = 2;
	type.gridy = 0;
	type.ipady = 2;
	...
	reference.setText(String.valueOf(details.get(5)));
	...
	ArrayList<String> addstr = (ArrayList<String>)details.get(6);
	...
	telephone.setText(String.valueOf(details.get(7)));
	...
	addpanel.add(sh.getGenericDateEdit(date, ul.ukFormat((java.sql.Date)details.get(10))), date1TextField);	
	...
	addpanel.add(sh.getDaySpinnerEdit(term, (String)details.get(11)), termTextField);
	...
	if (e.getSource().equals(ok)){
		...
		bean.connect().updateSubContractor(ul.writeArrayList(subdetails), ul.writeInteger(Integer.valueOf(sub_contractid)));		
	    dispose();
		}
    }
