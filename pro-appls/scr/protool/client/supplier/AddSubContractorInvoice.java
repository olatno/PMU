...
* @version 1.0
*/
public class AddSubContractorInvoice extends JFrame implements ActionListener{//implements ActionListener{
    private JTextField invoice;
    private JTextField account;
	private JComboBox vat;
    private JTextField projectId;
	private JTextField subContractorId;
    ...
    public AddSubContractorInvoice(String title, int subContractorIds){//, int proId
   	super(title); 
	...
	GridBagConstraints subContractorIdLabel = new GridBagConstraints();
	subContractorIdLabel.gridx = 0;
	subContractorIdLabel.gridy = 1;
	subContractorIdLabel.weighty =  weightye;
    subContractorIdLabel.anchor =  GridBagConstraints.EAST;
	addpanel.add(new JLabel("Sub-contractor Id:"), subContractorIdLabel);

	GridBagConstraints subContractorIdField = new GridBagConstraints();
	subContractorId = new JTextField(textFieldColunm);
	subContractorIdField.gridx = 1;
	subContractorIdField.gridy = 1;
	subContractorIdField.ipady = 2;
	subContractorIdField.ipadx = 115;
	subContractorIdField.anchor =  GridBagConstraints.WEST;
	addpanel.add(subContractorId, subContractorIdField);
	subContractorId.setText(String.valueOf(subContractorIds));
	subContractorId.setEditable(false);
	...
	account.setText(accname.get(new Integer(50020)));
   ...
   public void actionPerformed( ActionEvent e) {  
	...
	if (e.getSource().equals( addproject)){
	 ...
	 if (e.getSource().equals(ok)){
		java.util.List<Object> contractorinvoice =  new ArrayList<Object>();
		contractorinvoice.add(projectIds);
		contractorinvoice.add(Integer.valueOf(subContractorId.getText()));
		contractorinvoice.add(Integer.valueOf(50020));
		contractorinvoice.add(invoice.getText());
		contractorinvoice.add(ul.usDate(sh.dateString(date)));
		contractorinvoice.add(invoiceAmount.getText());
		contractorinvoice.add(vat.getSelectedItem());
	    bean.connect().addSubContractorInvoice(ul.writeArrayList(contractorinvoice));    
	    dispose();
	 ...
	 if (e.getSource().equals(save)){
	    ...
	    bean.connect().addSubContractorInvoice(ul.writeArrayList(contractorinvoice)); 
		dispose();
		AddSubContractorInvoice ds = new AddSubContractorInvoice("SUPLIER INVOICE", (new Integer(subContractorId.getText())).intValue());
