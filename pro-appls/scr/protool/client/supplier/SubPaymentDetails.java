...
public class SubPaymentDetails extends  JFrame implements ActionListener, ListSelectionListener{//ItemListener implements Serializable, ActionListener
    ...
    private final String[]columnNames = {"DATE","PAYMENT(£)","VAT(£)","CIS(£)","DATE","Amount(£)", " "};//"DATE","RECEIPT(£)","QUOTE(£)"
   

    public SubPaymentDetails(String title, Outgoingview view) {
	super(title);
	this.view = view;
	...
	//ledgerTablePanel.add(new JLabel(String.format("%s %s", "INVOICE NO", view.getStringValue(2))),BorderLayout.NORTH);
	...
	ledgerTablePanel.add(new JLabel(String.format("%s %s", "INVOICE NO", view.getStringValue(2, view.getModel(), view.getTable()))),BorderLayout.NORTH);
	...
	pu.subPaymentDetails(ledgerDataModel, view);
    }
	...
	if (e.getSource().equals(exit)){
	 ...
	 if (e.getSource().equals(refresh)){
		 pu.subPaymentDetails(ledgerDataModel, ((Integer)ledgerDataModel.getValueAt(0, 6)).intValue(), view);
	  }
    ...
    public void valueChanged(ListSelectionEvent e) {
		...
		if(maintablerow > 0 && maintablerow <= pu.subArrayListOfObjects(paymentId).size()){
			type.setText(String.valueOf(pu.subArrayListOfObjects(paymentId).get(maintablerow-1).get(4)));
			note.setText(String.valueOf(pu.subArrayListOfObjects(paymentId).get(maintablerow-1).get(5)));
		}
		else{
			type.setText("");
			note.setText("");
